...
def leader(infa, injsonSec, injsonGG, outf, threshold):
	...
	if os.path.getsize(infa) > 0:
		...
		df_signalp6_m = pd.merge(df_pre_sec, df_signalp6[["# ID", "CS Position"]], how='left', left_on="ID", right_on="# ID")
		df_signalp6_m[['leader_sec', 'core_sec']] = df_signalp6_m.apply(lambda row: split_leader(row['Sequence'], row['CS Position']), axis=1, result_type='expand')
		df_signalp6_f = df_signalp6_m[['ID', 'Sequence', 'leader_sec', 'core_sec']]
		df_signalp6_f = df_signalp6_f.rename(columns={'ID': 'Uniq_ID'})
	else:
		df_signalp6_f = pd.DataFrame(columns=['Uniq_ID', 'Sequence', 'leader_sec', 'core_sec'])
	...
	if list_pre_gg: # judge whether the input is empty
		...
		for d in cleavage_predictions:
			...
			list_gg.append({"Uniq_ID":seqname, "Sequence": sequence, "leader_gg": leader_gg, "core_gg": core_gg})
