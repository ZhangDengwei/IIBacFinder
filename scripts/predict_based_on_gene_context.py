...
def merge(inList):
	...
	merge_list_ordered = [sorted(x, key=lambda y: int(y.split("_")[-1])) for x in merge_list]
...
def summarize(hmmResult, outDir):
	...
	inclusion_rules = ["Peptidase_C39", "EntA_Immun", "DUF5841", "bPH_2", "DUF1430", "Rce1-like", "DUF5976", "DUF5836",
						"imm_Aci221A", "imm_Aci221B", "imm_BM1I", "imm_ORF16", "imm_ORF2", "imm_OrfB", "imm_abpIM",
						"imm_bacB_32", "imm_bacB_43", "imm_bimlp", "imm_bimlp2", "imm_brcI", "imm_cbiA", "imm_cbiB2",
						"imm_dviA", "imm_entCI", "imm_entiP", "imm_entqC", "imm_gakI", "imm_lafI", "imm_lagC", "imm_lciM",
						"imm_lebI", "imm_papB", "imm_plnc8C", "imm_saiA", "imm_sakTIM", "imm_spiA", "imm_spiQ"]
	protease_related = ["Peptidase_C39"]
	immunity_related = ["EntA_Immun", "DUF1430", "Rce1-like",
						"imm_Aci221A", "imm_Aci221B", "imm_BM1I", "imm_ORF16", "imm_ORF2", "imm_OrfB", "imm_abpIM",
						"imm_bacB_32", "imm_bacB_43", "imm_bimlp", "imm_bimlp2", "imm_brcI", "imm_cbiA", "imm_cbiB2",
						"imm_dviA", "imm_entCI", "imm_entiP", "imm_entqC", "imm_gakI", "imm_lafI", "imm_lagC", "imm_lciM",
						"imm_lebI", "imm_papB", "imm_plnc8C", "imm_saiA", "imm_sakTIM", "imm_spiA", "imm_spiQ"]
...
def hmmscan(hmmcmd, hmmdomain, outDir, inFasta, database):
	...
	command = f"{hmmcmd} --tblout {out_prediction} --cut_ga {hmmdomain} {inFasta} > /dev/null"
	subprocess.run(command, shell=True)
...
def geneContextPredict(inFasta, HMMexcute, amPEP, Rcmd, hmmcmd, pfamDomain, ncbiDomain, outDir):
	'''
	inFasta: input FASTA
	outDir: output dir
	'''
	...
	gene_candidates, gene_rules, dict_geneTypes = summarize(output_hmmsearch, out_hmmsearch_parase)
	...
	with open(summary_final, "w") as fo:
		header = ["CDs", "Prediction_rules", "PFAM_domain", "NCBI_domain", "Sequence", "Length"]
		...
		if gene_candidates:
			...
			out_candidates = os.path.join(outDir, "hmm_geneContext_parase", identifier+".fa")
			fasta_seq, gene_prediction = fastaSeq(inFasta, gene_candidates, out_candidates)
			# Length of flanking genes may be >150aa
			if gene_prediction:
				#-------------4. Excuting amPEP
				ampep_AMP_list = ampepExcute(amPEP, amPEPmodel, out_hmmsearch_parase, out_candidates)
				...
				#-------------6. Run hmmscan to annotate the PFAM domians of candidates
				dict_hmmscan_pfam = hmmscan(hmmcmd, pfamDomain, out_hmmsearch, out_candidates, "pfam")
				dict_hmmscan_ncbi = hmmscan(hmmcmd, ncbiDomain, out_hmmsearch, out_candidates, "ncbi")
				...
				for classII_candidate in overall_AMP:
					...
					if not list(set(exclusion_domains).intersection(set(both_domain))):
						...
						print(*[classII_candidate, predict_rule, pfam_domain, ncbi_domain, seq, len(seq)], sep="\t", file=fo)
...
def main():
	parse = argparse.ArgumentParser(description="Annotating class II bacteriocins based on gene context rules")

	parse.add_argument("--infasta", help="Input FASTA file for annotation", required=True)
	parse.add_argument("--hmmexcute", help="The excutive hmmsearch, defualt: hmmsearch", default="hmmsearch", required=False)
	parse.add_argument("--ampep", help="The excutive ampep, defualt: ampep", default="ampep", required=False)
	parse.add_argument("--Rcmd", help="The excutive Rscript, defualt: Rscript", default="Rscript", required=False)
	parse.add_argument("--hmmscan", help="The excutive hmmscan, defualt: hmmscan", default="hmmscan", required=False)
	...
	geneContextPredict(args.infasta, args.hmmexcute, args.ampep, args.Rcmd, args.hmmscan, args.pfamDomain, args.ncbiDomain, args.outDir)
