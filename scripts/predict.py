...
# @Last Modified time: 2024-04-19 14:28:21
...
def run_pyrodigal(model, outDir, fasta):
	'''
	Call Pyrodigal with a minimum CDS length of 10 amino acids (30 nt).
	...
	if not records:
		return
	min_cds_aa = 10
	# 10 aa = 30 nt; max_overlap must be smaller than min_gene.
	gene_finder = pyrodigal.GeneFinder(
		meta=(model == "meta"),
		min_gene=min_cds_aa * 3,
		min_edge_gene=min_cds_aa * 3,
		max_overlap=20,
	)
	if model == "single":
		total_len = sum(len(record.seq) for record in records)
		if total_len >= 20000:
			gene_finder.train(*[str(record.seq) for record in records])
		else:
			logging.warning("Input shorter than 20 kb; using Pyrodigal metagenomic mode for gene calling.")
			gene_finder = pyrodigal.GeneFinder(meta=True, min_gene=min_cds_aa * 3, min_edge_gene=min_cds_aa * 3, max_overlap=20)
	with open(out_f1, "w") as faa_out, open(out_f2, "w") as fna_out, open(out_f3, "w") as gbk_out:
		for record in records:
			genes = gene_finder.find_genes(str(record.seq))
			if not genes:
				continue
			genes.write_translations(faa_out, record.id)
			genes.write_genes(fna_out, record.id)
			genes.write_genbank(gbk_out, record.id)
	...
	for root, dirs, files in os.walk(outRegion):
		for file in files:
			if file.endswith(".tsv"):
				...
				q = list(set(q))
				q.sort()
				df_region_s = df_region[["Genome", "Region", "Contig", "CDs", "Start", "End", "Strand", "Partial_index", "Start_type", "RBS_motif"]]

				# df_region_s.loc[:,"Including_elements"] = ",".join(q)
				df_region_s.insert(df_region_s.shape[1], 'Including_elements', ",".join(q))
	...
	if isinstance(rbs, str):
		if partial == 'complete':
			if rule == 'Both':
				...
				if leaderType in ['Sec-dependent', 'Double-glycine', 'Double-glycine or Sec-dependent']:
					if matureP:
						confidence = 'High'
					else:
						confidence = 'Low'
...
def predict(hmmexcute, ampep, Rcmd, hmmscan, pfamDomain, ncbiDomain, outDir, threshold, fasta):
	...
	os.system(cmd_a)
	'''
	annotate_regions(final_gene_list, fasta, outDir, identifier, dict_geneTypes, hmmscan, pfamDomain, threshold)
	


def main():
	import sys
	parse = argparse.ArgumentParser(prog="IIBacFinder",
									description="Detecting class II bacteriocins from genomes")
	parse.add_argument('-i', "--inDir", help="Input path to folder containg FASTA file, whose suffix could in list of ['.fas', '.fa', '.fasta', '.faa', '.fna']", required=True)
	parse.add_argument('-e', "--hmmexcute", help="The excutive hmmsearch, defualt: hmmsearch", default="hmmsearch", required=False)
	parse.add_argument('-a', "--ampep", help="The excutive ampep, defualt: ampep", default="ampep", required=False)
	parse.add_argument('-r', "--Rcmd", help="The excutive Rscript, defualt: Rscript", default="Rscript", required=False)
	parse.add_argument('-s', "--hmmscan", help="The excutive hmmscan, defualt: hmmscan", default="hmmscan", required=False)
	# parse.add_argument("--pfamdomain", help="PFAM domain", required=True)
	parse.add_argument('-o', "--outDir", help="The path to the folder storing output files", required=True)
	parse.add_argument('-t', "--threshold", help="Number of threshols used, default: 20", type=int, default=20, required=False)
	...
	parse.add_argument('-p', "--pyrodigal", default=True, action="store_false", help="Whether perform gene calling with Pyrodigal (default: TRUE), toggle to close if input files are already protein FASTA." )
	parse.add_argument('-m', "--model", default="single", choices=["single", "meta"], help="Select the Pyrodigal gene calling model (single or meta). Default is single" )
	parse.add_argument('-v', "--version", action="version", version='IIBacFinder: v1.2', help="Print out the version and exit." )
	...
	logging.basicConfig(filename=log_file_path, level=logging.DEBUG, format=LOG_FORMAT, datefmt=DATE_FORMAT, filemode='w')
	...
	partial_predict = functools.partial(predict, args.hmmexcute, args.ampep, args.Rcmd, args.hmmscan, pfamdomain, ncbidomain, out_abs, args.threshold)

	if args.pyrodigal:
		pyrodigal_out = os.path.join(args.outDir, "pyrodigal_out")
		if not os.path.exists(pyrodigal_out):
			os.makedirs(pyrodigal_out)
		logging.info(f"Run Pyrodigal for gene calling (minimum CDS length: 10 aa)")
		partial_pyrodigal = functools.partial(run_pyrodigal, args.model, pyrodigal_out)
		print("##############################")
		print("Run Pyrodigal (minimum CDS length: 10 aa):")
		print("##############################")
		process_map(partial_pyrodigal, fasta_lists, max_workers=int(args.threshold), chunksize=1)
		# prediction list
		prediction_lists = scanDir_pyrodigal(pyrodigal_out)
		print("##############################")
		print("Run class II bacteriocin prediction:")
		print("##############################")
		process_map(partial_predict, prediction_lists, max_workers=int(args.threshold), chunksize=1)
	else:
		print("##############################")
		print("Run class II bacteriocin prediction:")
		print("##############################")
		process_map(partial_predict, fasta_lists, max_workers=int(args.threshold), chunksize=1)
	...
	if args.pyrodigal:
		for file in os.listdir(out_plot):
			if file.endswith("tsv"):
				...
				read_region(os.path.join(out_plot, file), genome_path, os.path.join(out_plot, outfile))
	...
	df_combine = pd.merge(result, result_region,  how='left', left_on=['Genome','CDs'], right_on = ['Genome','CDs'])
	df_combine['Uniq_ID'] = df_combine['Genome'].astype(str) + '_+_' + df_combine['CDs'].astype(str)
	logging.info(f"Total predicted precusors of class II bacteriocins: {df_combine.shape[0]}")
	...
	with open(out_fa, "w") as fo, open(out_sec_json, "w") as jse, open(out_gg_json, "w") as jgg,\
		open(out_sec_fa, 'w') as fos, open(out_gg_fa, 'w') as fog:
		# fin.readline()
		# for line in fin:
		...
		for index, row in df_combine.iterrows():
			...
			if row['Potential_Leader_Type'] == 'Sec-dependent':
				...
				print(seq, file=fog)
			elif row['Potential_Leader_Type'] in ['Double-glycine or Sec-dependent', 'Unclear']:
				json_dict_sec[uniqid] = seq
				print(">"+uniqid, file=fos)
				print(seq, file=fos)
				json_dict_gg[uniqid] = seq
	...
	if os.path.getsize(out_sec_fa) > 0 or os.path.getsize(out_gg_fa) > 0:
		df_leader = leader(out_sec_fa, out_sec_json, out_gg_json, sigoutdir, args.threshold)
		df_combine_leader = pd.merge(df_combine, df_leader, on=["Uniq_ID", "Sequence"], how='left')

		# Combine the leader prediction
		df_combine_leader['Predicted_mature_peptide'] = df_combine_leader.apply(lambda row: leader_combine(row['Potential_Leader_Type'], 
															row['Sequence'], 
															row['core_sec'], 
															row['core_gg']),
														 	axis=1, result_type='expand')
		# add confience level
		df_combine_leader['Confidence'] = df_combine_leader.apply(lambda row: confidenceP(row['RBS_motif'], 
														row['Partial_index'], 
														row['Rules'], 
