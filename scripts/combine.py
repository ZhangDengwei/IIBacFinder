...
def combine(domainRes, geneContextRes, out):
	...
	with open(out, "w") as fo:
		header = ["CDs", "Rules", "Domain_rule", "Domain_Evalue", "Domain_Bitscore", "Context_rule", "PFAM_domain", "NCBI_domain", "Sequence", "Length", "Description", "Potential_Leader_Type"]
		...
		if list_both:
			for gene in list_both:
				...
				out_c = [gene, "Both", domain_rule, domain_evalue, domain_bitscore, context_rule, pfam_domain, ncbi_domain, seq, seq_len, des_domain, leader_pre]
		...
		if list_d:
			for gene in list_d:
				...
				seq_len = df_d.loc[df_d.CDs==gene, "Length"].values[0]
				des_domain, leader_pre = dict_desp[domain_rule]
				out_c = [gene, "Domain", domain_rule, domain_evalue, domain_bitscore, "*", pfam_domain, ncbi_domain, seq, seq_len, des_domain, leader_pre]
				print(*out_c, sep="\t", file=fo)
		if list_g:
			for gene in list_g:
				context_rule = df_g.loc[df_g.CDs==gene, "Prediction_rules"].values[0]
				...
				if "Peptidase_C39" in context_rule:
					...
					if "EntA_Immun" in context_rule:
						...
						des_rule = "Association with lactococcin 972 family of bacteriocins"
				...
				out_c = [gene, "GeneContext", "*", "*", "*", context_rule, pfam_domain, ncbi_domain, seq, seq_len, des_rule, leader_pre]
				print(*out_c, sep="\t", file=fo)
...
def main():
	...
	parse.add_argument("--domainRes", help="The prdiction results based on domain rule", required=True)
	parse.add_argument("--geneContextRes", help="The prdiction results based on gene context rule", required=True)
