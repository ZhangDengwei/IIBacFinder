...
class DatasetGenerator:
    ...
    def _read_jsons(self):
        ...
        self.raw_data = [x for x in self.raw_data if len(x['sequence']) < 300]
    ...
    def train(self, epochs=100):
        ...
        with open(self.save_path / 'datasplit.json', "w") as fp:
            json.dump(name_data_map, fp, indent=4)
    ...
    def create_crf(self):
        ...
        #                       ('after', 'after'),
        #                       ('after', 'stop'),]
        allowed_transitions = [('start', 'before'),('before', 'before'),
                       ('before', 'prop'),
                       ('prop', 'prop'),
                       ('prop', 'stop'),
                       ('stop', 'pad'),
    ...
    def evaluate(self, ds):
        ...
        for batch in tqdm(batches):
            ...
            for batch_idx in range(len(batch)-1):
                #targs, pred = tgt[batch_idx], preds[batch_idx][0]
                targs, pred = tgt[batch_idx], preds[0][batch_idx]
                total_right = 0
    ...
    def evaluate_later(cls, model_path, vocab_path, datasplit_path, data_path):
        ...
        raw_data = [x for x in raw_data if len(x['sequence']) < 300]
