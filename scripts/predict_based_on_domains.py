...
def domainPredict(inFasta, HMMexcute, hmmscanExc, pfamDomain, ncbiDomain, outDir):
	...
	if dict_domain:
		...
		dict_hmmscan_pfam = hmmscan(hmmscanExc, pfamDomain, out_hmmsearch_parase, fasta_out, "pfam")
		dict_hmmscan_ncbi = hmmscan(hmmscanExc, ncbiDomain, out_hmmsearch_parase, fasta_out, "ncbi")
	...
	with open(out_parase_file, "w") as fo:
		header = ["CDs", "Annotation_Domain", "PFAM_domain", "NCBI_domain", "E_value", "Bit_Score", "Sequence", "Length"]
		...
		if dict_domain:
			for target, values in dict_domain.items():
				...
				if not list(set(exclusion_domains).intersection(set(list(both_domain)))):
					out = [target, domain_id, pfam_domain, ncbi_domain, evalue, hit_score]
...
def main():
	parse = argparse.ArgumentParser(description="Annotating class II bacteriocins with curated HMM domains")
	...
	parse.add_argument("--hmmscanExc", help="The excutive hmmscan, defualt: hmmscan", default="hmmscan", required=False)
	...
	parse.add_argument("--outDir", help="The path to the folder storing output files", required=True)

	args = parse.parse_args()

	domainPredict(args.inFasta, args.hmmexcute, args.hmmscanExc, args.pfamDomain, args.ncbiDomain, args.outDir)
