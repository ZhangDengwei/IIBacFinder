...
def fastaSeq(fasta, geneList, out):
	...
	with open(fasta) as fasta_file:
		...
		for seq in parser:
			...
			dict_fasta[seq.id] = [sequence, contig, start_p, end_p, strand, partial, start_type, rbs_motif]
	...
	with open(out_prediction, "r") as fin:
		for line in fin:
			if not line.startswith("#"):
				...
				full_domain_annotation = f"{target_acc}: {target_description} ({target_name})"
				if query not in dict_hmmscan:
					dict_hmmscan[query] = []
					dict_hmmscan[query].append([score, coord_from, coord_to, full_domain_annotation])
				else:
					append = True
...
def annotate_regions(geneList, fasta, outDir, out_prefix, gene_types, hmmcmd, hmmdomain, core=10):
	'''
	...
	for region in merged_regions:
		out_fa = os.path.join(out_annotation, out_prefix+"_region_"+"{:0>3d}".format(index)+".fa")
		...
		out_annotation_file = os.path.join(out_plot, out_prefix+"_region_"+"{:0>3d}".format(index)+".tsv")
		with open(out_annotation_file, "w") as fo:
			headers = ["Genome", "Region","Contig","CDs","Domain_index","Start","End","Strand","Orientation", "Partial_index", "Start_type", "RBS_motif",\
						 "Pfam_domain", "Domain_start", "Domain_end", "Domain_description","CDs_Class", "Sequence"]
			...
			for gene in region_order:
				...
				locus_tag = gene
				sequence, contig, start_p, end_p, raw_orientation, partial_index, start_type, rbs_motif = dict_fasta[locus_tag]
				orientation = "1" if raw_orientation=="1" else 0
				strand = "forward" if orientation=="1" else "reverse"
				if gene not in dict_hmmscan:
					pfam_domain, domain_start, domain_end, domain_description = "NA", "NA", "NA", "NA"
					if gene in gene_types:
						gene_class = gene_types[gene]
					else:
						gene_class = "others"
					domain_index_out = "NA"
					out_c = [out_prefix, file_name, contig, locus_tag, domain_index_out, start_p, end_p, strand, orientation, partial_index,\
						start_type, rbs_motif, pfam_domain, domain_start, domain_end, domain_description, gene_class, sequence]
					print(*out_c, sep="\t", file=fo)
				else:
					for domain in dict_hmmscan[gene]:
						domain_start, domain_end, domain_full_description = domain[1], domain[2], domain[3]
						...
						if gene in gene_types:
							...
							if re.search(r"regulator", domain_full_description, re.I):
								...
								gene_class = "immunity"
							elif re.search(r"transporter", domain_full_description, re.I):
								gene_class = "transporter"
							elif re.search(r"peptidase|protease", domain_full_description, re.I):
						...
						out_c = [out_prefix, file_name, contig, locus_tag, domain_index_out, start_p, end_p, strand, orientation, partial_index,\
						start_type, rbs_motif, pfam_domain, domain_start, domain_end, domain_full_description, gene_class, sequence]
						print(*out_c, sep="\t", file=fo)

		#--- plot region
		out_plot_file = os.path.join(out_plot, out_prefix+"_region_"+"{:0>3d}".format(index)+".svg")
		current_path = os.path.split(os.path.realpath(__file__))[0]
		script_path = os.path.join(current_path, "region_plot.R")
		command = f"Rscript {script_path} {out_annotation_file} {out_plot_file} > /dev/null"
		# subprocess.run(command, shell=True)
...
def main():
	parse = argparse.ArgumentParser(description="Summarizing, annotating, ploting regions of class II bacteriocins")

	parse.add_argument("--geneList", help="List of putative class II bacteriocin genes", required=True)
	...
	parse.add_argument("--core", help="Threshold used, default: 10", default=10, type=int, required=False)

	args = parse.parse_args()

	annotate_regions(args.geneList, args.fasta, args.outDir, args.out_prefix, args.gene_types, args.hmmscan, args.pfamdomain, args.core)
