...
# @Last Modified time: 2024-08-29 19:26:09
...
def alnDiamond(infa, db, out, core, annotation):
	...
	if os.path.getsize(out):
		df =  pd.read_table(out, header=None, names=["qseqid", "sseqid", "pident", "length", "qlen", "slen", "qcovhsp", "scovhsp", "evalue", "full_qseq", "full_sseq"])
		df[['Genome', 'CDs']] = df['qseqid'].str.split('_\+_', expand=True)
		df["query_coverage"] = df['qcovhsp'].astype(str) + ' (' + df['length'].astype(str) + '/' + df['qlen'].astype(str) + ')'
		df["target_coverage"] = df['scovhsp'].astype(str) + ' (' + df['length'].astype(str) + '/' + df['slen'].astype(str) + ')'
		# merge annotation
		df_anno = pd.read_table(annotation)

		df_m = pd.merge(df, df_anno,  how='left', left_on='sseqid', right_on='Accession')
		df_m_s = df_m[["Genome", "CDs", "length", "pident", "query_coverage", "target_coverage", "full_sseq", "Accession", "Name", "Database"]]
		df_m_s = df_m_s.rename({'pident': 'Hsp_identity', 
								'length': 'Hsp_len', 
