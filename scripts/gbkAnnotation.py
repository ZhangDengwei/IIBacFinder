...
def read_region(region, fasta, outFile):
	...
	with open(region, "r") as fin:
		...
		for line in fin:
			...
			cds_start = int(items[5]) - start_p + 1 if strand_re == -1 else int(items[5]) - start_p
			...
			# add CDs annotation
			cds_feature = SeqFeature(FeatureLocation(ExactPosition(cds_start), ExactPosition(cds_end), strand=strand_re), 
										type='CDS',
										qualifiers={"locus_tag":cds_id, "gene_kind":gene_kind, "product":cds_class, "partial_index":partial_index})
			...
			if domain_description != "NA":
				...
				pfam_id = items[12]
				pfam_des = items[15]
				domain_feature = SeqFeature(FeatureLocation(ExactPosition(domain_start), ExactPosition(domain_end)), 
										type='domain',
										id=pfam_id,
